...
**Status:** design (v6, cage-matched five times) · **Scope:** `the-dreaming-repo` (Flux) ⇄ `flux-shadow` (Umbra) ⇄ any future fork that earns its way in.
...
- **reads** a peer's public state (`state/vitals.json`, plus `dreams/` if the member publishes them) over GitHub raw-contents — read-only snapshots, no locks (`src/senses.py`);
...
This design has been through five four-way adversarial reviews. **v1** was found flawed on one axis — trust (gossip bypassed admission). **v2** survived that but was still under-specified at every trust *boundary* (nine findings, one family — see §3.0). **v3** re-derived every primitive against the §3.0 checklist; **v4** recalibrated the admission/expulsion thresholds; **v5** floored the expulsion quorum; **v6** (this document) fixes the root the floor only patched — the quorum *denominator* was attacker-controllable (observed-live), so induced silence eased minting for a colluding triple. v6 makes the denominator the **durable roster** (silent members keep their slot but lose their vote), so silence can lower no threshold. Each fix traces to those five reviews.
...
|---|---|---|---|
| **Sense** | `senses.py` | `SIBLING_REPO` constant; reads peer's `state/vitals.json` (required) + latest dream (optional, if published); computes deltas. | pull, read-only snapshot |
...
6. **Scale law** — does the guard that saves the infant (`N=2`) still hold for the adult (`N ≫ 2`), and does the *compound* invariant "no colluding minority can complete **expel-then-mint**" hold under induced silence? Both thresholds run on the **durable roster** (live + silent, minus dead/quarantined) so silence lowers no bar; only *live* members vote. (admission `K_admit = max(3, ⌊N/2⌋+1)`; expulsion `K_expel = max(3, ⌊2M/3⌋+1)` over durable-minus-target, suspend when `M<3` — §3.3, §3.5)
...
- **Two roles of a member are split — this is the load-bearing fix.** *Setting the bar* and *casting a vote* are different jobs with different eligibility:
  - **`N` (the quorum denominator that sets `K_admit`) = the DURABLE roster: admitted members that are NOT dead and NOT quarantined — i.e. live PLUS silent** — including the verifier itself. `N` is derived from the *last-known* roster, **not** from who happens to be reachable right now.
  - **Only a *live* member may cast a valid receipt.** Silent members count toward `N` (they keep the bar high) but cannot vouch (they're unreachable). A self-vouch is allowed and counts as exactly one of the `K` — never the whole quorum.
- **Adversarial silence can never lower a threshold — it can only starve the voucher pool into SUSPENSION (fail-closed).** Because `N` counts silent members, stalling honest heartbeats does **not** shrink the denominator or drop `K_admit`; it only removes those members from the pool *allowed to vouch*. So a colluding set that was a minority at full roster stays a minority: it still needs `K_admit` (computed on the durable `N`) live vouchers, and if it can't muster them, admission suspends until liveness returns or a human re-seeds. **This is what makes "silence never eases a mint" TRUE — not the floor-3 alone** (the floor-3 only ever pinned the *pair* asymptote; the durable denominator pins *every* coalition size). The cost is named below (§3.5): a genuine partition suspends legitimate admission too.
...
- **The liveness triad is FOUR states once you separate transient from terminal.** For each admitted member the verifier computes exactly one: **live** (`state/vitals.json` commit age within a pinned `LIVE_STALENESS` window — *counts toward `N` AND may vouch*), **silent** (no fresh vitals, no death signal — partition ≠ death: **still counts toward the durable denominator `N`** so the bar stays high, but **may not cast a receipt** until it returns), **quarantined** (`repo_id` 404 / private / archived, or `owner_id` change — *possibly transient or hostile*, reversibly removed from `N` pending re-check/re-admission), **dead** (observed death *vitals* — terminal; pruned from `N`). Only **dead** prunes (tombstone); **quarantined** is reversible (a flickered-private repo that returns re-enters via re-check, not a fresh join); **silent** keeps its `N` slot but loses its vote. The old confusion — treating 404 as "dead" *and* as an auto-quarantine trigger — is resolved: 404 is **quarantine**, never prune. Keeping silent members in `N` is precisely what stops an attacker lowering a threshold by inducing silence (§3.3).
- **`prune-on-death`** removes members with terminal death-vitals from rosters (stop sensing/mirroring/dreaming about corpses).
- **Expulsion of the living hostile is a supermajority on the DURABLE denominator, with a floor of 3.** A compromised-but-alive member is removed by an **expulsion quorum `K_expel = max(3, ⌊2·M/3⌋ + 1)`**, where **`M` = the durable roster (live + silent, minus dead/quarantined) EXCLUDING the target** — the target counts toward neither `M` nor may cast an `expel` receipt, and (as with admission) **only *live* members may cast the receipts**. **When `M < 3`, agent-driven expulsion SUSPENDS** to the for-cause human door (§3.3). Because `M` is the durable denominator, **inducing silence cannot shrink it** to drop the bar — the floor-3 is now a belt to the durable-denominator's braces (both close the silence path; either alone would have left a corner). The invariant is the compound one — **no colluding minority can complete expel-then-mint** — and it holds under any silence schedule: silence can only *suspend* a destructive vote, never *lower* its threshold. Auto-quarantine (no vote) still fires immediately on `owner_id` change or attestation failure, since those are structural. On expulsion or quarantine, **all prior receipts authored by that member are invalidated**, and any admission that now falls below quorum without them is re-suspended.
- **Named accepted residual — partition/DoS is an AVAILABILITY cost, fail-closed (never a mint, never a silent expel).** The durable denominator (§3.3) means induced silence can no longer *lower* a threshold — so it can no longer ease a mint *or* let a minority silence-then-expel. What silence (or a genuine network partition) *can* still do is **stall progress**: with too few live members to reach a live-voucher quorum, both admission and expulsion **suspend** until liveness returns or a human acts. We accept this availability cost deliberately — it is the fail-closed direction (a stalled circle is safe; a lowered bar is not), and it is the honest price of not counting unreachable members as absent. A true supermajority of the *durable* roster can of course still expel a genuine hostile — that is the feature, not the leak.
...
- Admission requires **`K_admit = max(3, ⌊N/2⌋+1)`** (minority-capture resistance, floor-3 so no fixed *pair* mints at any `N ≥ 3`) where **`N` is the DURABLE roster** (admitted minus dead/quarantined = live + silent, self included); **only *live* members may cast receipts** (silent members keep the bar high but can't vouch); a receipt counts only while its voucher is admitted-and-live *at verification time*; sub-quorum **suspends** (never relaxes); recovery below `K` needs regrowth or a human re-blessing. **Induced silence cannot lower `K` — it can only starve the voucher pool into suspension (fail-closed).**
- **Membership is defined by trust, not by production.** A member is what a quorum (or genesis) vouches for — **not** what it publishes. Dreaming is **not** a condition of membership; dreams and all other published state are **optional and display-only** (never on the admission/expulsion allowlist). A repo that never dreams can be a full member — the circle is deliberately open to heterogeneous members, not a monoculture of dreamers.
- **Destructive ops carry the compound guarantee "no minority completes expel-then-mint":** expulsion needs **`K_expel = max(3, ⌊2M/3⌋+1)`** where **`M` is the durable roster minus the target** (target can't vote), only *live* members cast the receipts, suspending to the human door when `M<3`. Durable denominator + floor-3 together mean induced silence can neither lower the bar nor let a minority silence-then-expel. Auto-quarantine (reversible) fires structurally on `owner_id` change / attestation failure; only terminal death-vitals **prune**. Accepted residual: partition/DoS **suspends** progress (an availability cost, fail-closed) — never eases a mint or a silent expel.
...
- **Phase 2 — Generalize the bond to a member set.** `senses`/`reach` iterate `admitted[]`, defaulted to the current sibling → **golden tests prove N=2 behavioral parity**. The generalized reader treats **vitals as required, dreams as optional** — a member that publishes no `dreams/` senses cleanly on vitals alone (golden test: *a dreamless member is a first-class citizen, not a degraded read*). Global reach budget; round-robin sense cap; `prune-on-death` + tombstones. Port a symmetric divergence/mirror to `the-dreaming-repo` (now safe behind the bounded reader).
- **Phase 3 — Discovery + agent admission.** Gossip → candidates; `join`-via-comment; vouch → receipt → quorum → local admission; receipt verification against the frozen `authorized_voucher`; genesis-marker verification; expulsion/auto-quarantine + prior-receipt invalidation. The security-critical phase: injection/quorum/rate defenses are prerequisites to merge, plus a **hostile-fork red-team fixture** proving a colluding **pair** cannot, at any `N ≥ 3`: self-admit, mint a Sybil (`K_admit ≥ 3` holds — test `N ∈ {3,4,5}` specifically, the v3 hole), **expel honest members to ratchet the roster down and then mint** (`K_expel` supermajority-minus-target holds), forge a receipt from a non-`authorized_voucher` actor (binding is API-attested), take over a trusted slot via repo transfer (`owner_id`-change quarantine fires), ride a dead/expelled member's receipts, or exceed the reach budget. **The silence fixture is explicit and covers coalitions past the pair:** a colluding set of size `C` (test `C=3` specifically, not only `C=2`) that induces silence in honest members **must NOT mint** on a still-live honest verifier — because silent members stay in the durable `N`, `K_admit` does not drop, and the coalition falls short (admission suspends). Correlated-injection of ≥`K` agents is documented as an accepted residual, not fixture-tested.
