...
**Status:** design (v5, cage-matched four times) · **Scope:** `the-dreaming-repo` (Flux) ⇄ `flux-shadow` (Umbra) ⇄ any future fork that earns its way in.
...
This design has been through four four-way adversarial reviews. **v1** was found flawed on one axis — trust (gossip bypassed admission). **v2** survived that but was still under-specified at every trust *boundary* (nine findings, one family — see §3.0). **v3** re-derived every primitive against the §3.0 checklist; **v4** recalibrated the admission/expulsion thresholds; **v5** (this document) floors the expulsion quorum so induced silence can't leak it, and states the true *compound* invariant (no minority completes expel-then-mint). Each fix traces to those four reviews.
...
6. **Scale law** — does the guard that saves the infant (`N=2`) still hold for the adult (`N ≫ 2`), and does the *compound* invariant "no colluding minority can complete **expel-then-mint**" hold under induced silence? (admission `K_admit = max(3, ⌊N/2⌋+1)`; expulsion `K_expel = max(3, ⌊2M/3⌋+1)` over live-minus-target, suspend when `M<3` — §3.3, §3.5)
...
- **Expulsion of the living hostile is a supermajority WITH a floor of 3.** A compromised-but-alive member is removed by an **expulsion quorum `K_expel = max(3, ⌊2·M/3⌋ + 1)`** over the `M` live admitted members **excluding the target** (the target neither counts toward `M` nor may cast an `expel` receipt); **when `M < 3`, agent-driven expulsion SUSPENDS** and removal falls to the for-cause human door (§3.3). The floor-3 mirrors admission's, and for the same reason: without it, an attacker who induces honest **silence** shrinks `M` until `⌊2M/3⌋+1` drops to 2 and a colluding pair could expel a live honest member. The **precise invariant this protects is the compound one — no colluding minority can complete expel-then-mint** — not the looser (and, at some `N`, false) claim that `K_expel` is always numerically greater than `K_admit`; every culling path bottoms out at `N < K_admit`, where admission suspends (the v3 capture ramp N=5,K=2 → N=2 Sybil farm is closed at both ends). Auto-quarantine (no vote) still fires immediately on `owner_id` change or attestation failure, since those are structural, not a judgement call. On expulsion or quarantine, **all prior receipts authored by that member are invalidated**, and any admission that now falls below quorum without them is re-suspended.
- **Named accepted residual — silence-assisted culling (an availability limit, never a mint).** Even with the floor, a supermajority of the observed-live set (or a pair once the floor forces suspension into the human door) can *deny* an honest member its slot by expelling or DoS-ing it — forcing a human for-cause re-seed. This is the destructive dual of the accepted dread-edge collusion: a hostile-enough live majority can *cull*, never *farm*. We accept the availability cost (it always terminates at a human door) rather than add anti-collusion machinery that would also block legitimate expulsion of a real hostile.
...
- **Destructive ops carry the compound guarantee "no minority completes expel-then-mint":** expulsion needs **`K_expel = max(3, ⌊2M/3⌋+1)`** over live members **excluding the target** (target can't vote), suspending to the human door when `M<3` — the floor-3 mirrors admission's so induced silence can't drop the bar to a pair. Every culling path ends at `N < K_admit` (suspend). Auto-quarantine (reversible) fires structurally on `owner_id` change / attestation failure; only terminal death-vitals **prune**. Accepted residual: a hostile live majority can *cull* (deny a slot, forcing a human re-seed), never *farm*.
