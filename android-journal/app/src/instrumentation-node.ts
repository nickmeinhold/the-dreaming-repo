/**
 * Node-Runtime Instrumentation Body
 *
 * Dynamically imported by instrumentation.ts ONLY in the nodejs runtime.
 * Must never be statically imported: it pulls in the pino logger, whose
 * module-scope buildStream() requires node:fs — a throwing stub in the
 * Edge bundle (this crashed every request in the standalone build).
 *
 * Logs startup context (Node version, environment, DB connectivity),
 * registers process-level exception handlers, and validates critical
