...
/** Convert a Result to the { success, error, ...value } shape expected by client components */
export function toActionResult<T extends Record<string, unknown>>(
  result: Result<T, string>,
): { success: boolean; error?: string } & Partial<T> {
  if (result.isOk()) {
    return { success: true, ...result.value } as { success: boolean; error?: string } & Partial<T>;
  }
  return { success: false, error: result.error } as { success: boolean; error?: string } & Partial<T>;
}
